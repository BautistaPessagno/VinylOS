...
test("search normalization is deterministic across client locales", () => {
  const filename = fileURLToPath(new URL("./searchQuery.ts", import.meta.url));
  const source = readFileSync(filename, "utf8");
...
test("search errors preserve Discogs rate-limit guidance and hide other internals", () => {
  const { searchErrorMessage } = loadSearchQuery();

  assert.equal(
    searchErrorMessage(new Error("Discogs rate limit exceeded, try again shortly.")),
    "Discogs rate limit exceeded, try again shortly.",
  );
  assert.equal(
    searchErrorMessage(new Error("Discogs API error 500: private upstream details")),
